...
async function fetchOverall(characterName, world) {
  ...
  const region = getRegion(world);
  const url = `${NEXON_BASE[region]}?type=world&id=${worldId}&reboot_index=2&page_index=1&character_name=${encodeURIComponent(characterName)}`;
  try {
...
module.exports = { ALLOWED_WORLDS, fetchOverall, fetchFame };
